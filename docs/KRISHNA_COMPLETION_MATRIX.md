...
| Local Model Scout | IMPLEMENTED / RUNTIME VERIFY | Persistent local candidate evaluation scores quality, latency, RAM/VRAM fit and duplication without downloading models or gaining cloud-billing/routing authority. Activation remains a separate governed decision. |
...
| Rishi Vishvakarma design knowledge | IMPLEMENTED / RUNTIME VERIFY | Permanent council profile plus provenance-first candidate/verified design knowledge store and curriculum are wired. Modern UI/software-craft roles are explicit KRISHNA design assignments, not historical-practice claims. |
| Sudarshan Design Engine | IMPLEMENTED / RUNTIME VERIFY | Selective design skills, design-genome/drift comparison, hard acceptance gates and bounded repair/retest are wired through Shared Actions and the Sudarshan project lifecycle. |
| Local design-tool adapters | IMPLEMENTED BOUNDARY / ENVIRONMENT VERIFY | Playwright CLI detection, Storybook state contract and opt-in Stagehand recovery exist as adapters; none become a competing browser or design authority. |
