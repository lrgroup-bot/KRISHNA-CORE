...
| MRITYUNJAY permanent self-heal supervisor | IMPLEMENTED / RUNTIME VERIFY | Objective Action Bus / mission / tool failures queue bounded self-heal. Candidate-only repair remains test/runtime/UI verified. Low-risk edits to existing non-sensitive files may autonomously commit, request graceful Guardian restart, pass verified deployment/runtime acceptance, then push the canonical branch; failed deployment/push rolls back to the prior clean commit. Add/delete/control-plane/security/privacy/auth/credential/dependency/deployment/mobile changes remain quarantined. |
...
| HAWKEYE Learning Observer | IMPLEMENTED / DEVICE + RUNTIME VERIFY | User-selected camera/video/book/page/object/audio observations are distilled into provenance-linked findings, routed through Universal Learning -> BRAHMA -> the selected Rishi team, and kept as candidates until normal verification. Raw media is not promoted as knowledge. |
| Public research boundary | IMPLEMENTED / DEVICE VERIFY | HAWKEYE opens research in a user-browser Custom Tab with no KRISHNA JavaScript bridge on third-party content. Unknown faces cannot become social identity-search keys; explicit identity/enrolled-consented matches/public textual clues may seed normal public research. |
...
| Face identity boundary | IMPLEMENTED POLICY / ADAPTER REQUIRED | Unknown people remain UNKNOWN. Identity matching is limited to explicitly enrolled/consented local profiles; no cloud biometric provider is claimed and an unknown face/embedding may not be used to discover social-media identity. |
| Mobile object layer / auto-zoom | IMPLEMENTED SOFTWARE / DEVICE VERIFY | Bundled ML Kit object detection/tracking supplies local OBSERVED bounding boxes and tracking IDs; the mobile overlay draws them and requests camera-track zoom only when the device/browser exposes a zoom capability. Coarse labels are not exact identity claims. |
| Local PHOTO+DATA / REC+DATA | IMPLEMENTED SOFTWARE / DEVICE VERIFY | User-triggered photo and bounded short-video capture save locally through MediaStore (or app storage fallback) with a metadata sidecar containing session/goal/analysis/object boxes/zoom and explicit no-auto-cloud policy. |
...
| Architecture Truth Audit | IMPLEMENTED / RUNTIME VERIFY | `architecture_truth.py` reports legacy roots, duplicate basenames/content, missing requirement evidence, stale source-tree entries and conservative orphan candidates. Runtime acceptance now requires schema/version agreement between `/api/requirements` and `/api/architecture/truth`, fails on missing indexed evidence/invalid statuses, and surfaces duplicate/orphan/source-tree drift as review-only warnings. It never auto-deletes candidates. |
...
| React/shadcn foundation | IMPLEMENTED FOUNDATION / CI + RUNTIME VERIFY | `app/spatial-ui` is a React 19/Vite/TypeScript shell with local composable UI primitives, CVA/twMerge support and no dependency on the legacy transition HTML for its build. It remains opt-in/preview until operational project/chat/NARAD parity is accepted; the verified legacy shell stays default meanwhile. |
...
| Sudarshan Design Engine + Vishvakarma design knowledge | IMPLEMENTED / RUNTIME VERIFY | Structured Design Genome including accessibility, nested drift detection, bounded repair/retest, hard acceptance gates, provenance-preserving Vishvakarma learning/curation, permanent Rishi Council registration, live Playwright/Storybook/opt-in Stagehand adapters, Shared Action controls, read-only status APIs and the internal Spatial UI Design Intelligence panel are wired without adding sidebar clutter. Knowledge-bound UI work blocks until task-relevant verified Vishvakarma knowledge exists. |
| Structured Sudarshan Project Brain | IMPLEMENTED / RUNTIME VERIFY | Runtime-owned PROJECT/ARCHITECTURE/RULES/PHASES/DESIGN/AGENTS/MEMORY governance layers coexist with DB-backed verified learning; Shared Action provision/status/record and /api/project-brain are wired, and source-tree mutation is explicitly false. |
