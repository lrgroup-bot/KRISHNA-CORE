...
def main() -> int:
    parser=argparse.ArgumentParser(description='KRISHNA isolated Sanskrit recitation worker')
    ...
    proc=subprocess.run(
        command,cwd=str(engine_root),capture_output=True,text=True,
    ...
    print(json.dumps({
        'output':str(output),'language':'sa','provider':'edge-sanskrit-tts',
        'meter':str(args.meter or 'anushtubh'),'nfe':nfe,
    },ensure_ascii=False))
    return 0
