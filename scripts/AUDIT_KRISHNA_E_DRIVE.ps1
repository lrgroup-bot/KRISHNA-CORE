...
function Get-ProtectedEvidence([string]$Root){
  ...
  $pattern='(?i)(^|\\)(krishna_core\.db$|state(\\|$)|config(\\|$)|logs(\\|$)|reports(\\|$)|backups(\\|$)|knowledge(\\|$)|gyan[-_ ]?bhandar(\\|$)|ollama-models(\\|$)|models(\\|$)|avatar(\\|$)|assets\\avatar(\\|$)|certificates?(\\|$)|certs?(\\|$)|secrets?(\\|$)|credentials?(\\|$)|trusted[^\\]*device|pairing|\.env($|\.)|[^\\]+\.(pem|key|pfx|p12|cer|crt)$)'
...
foreach($port in @(8765,8766,8876,11434)){
