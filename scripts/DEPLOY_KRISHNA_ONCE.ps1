...
function Get-KrishnaListenerOwnership([int]$ProcessId,[string]$RuntimeRoot){
  ...
  $listener=$chain[0]
  if(([string]$listener.name) -notmatch '(?i)^python(?:\.exe)?
...
if(!$SkipStart){
  ...
  # A stale PID file can miss an orphaned venv/base-Python listener. In that case,
  # verify ownership from the full Windows parent chain before taking over.
  # Unknown listeners still fail closed and are never terminated.
  ...
  if($staleListener){
    ...
    if($ownership){
      $rootPid=[int]$ownership.verified_root_pid
      Write-Host ("Verified orphaned KRISHNA Core listener PID {0}; taking over verified process tree rooted at PID {1} ({2})." -f $listenerPid,$rootPid,$ownership.verified_root_name) -ForegroundColor Yellow
      & taskkill.exe /PID $rootPid /T /F | Out-Null
      if($LASTEXITCODE -ne 0 -and (Get-Process -Id $rootPid -ErrorAction SilentlyContinue)){
        throw ("Failed to stop verified orphaned KRISHNA Core tree rooted at PID {0}" -f $rootPid)
      }
      $released=$false
      for($i=0;$i -lt 20;$i++){
        Start-Sleep -Milliseconds 500
        $remaining=Get-NetTCPConnection -LocalPort 8766 -State Listen -ErrorAction SilentlyContinue | Select-Object -First 1
        if(!$remaining){$released=$true;break}
      }
      if(!$released){
        $remaining=Get-NetTCPConnection -LocalPort 8766 -State Listen -ErrorAction SilentlyContinue | Select-Object -First 1
        $remainingPid=if($remaining){[int]$remaining.OwningProcess}else{0}
        throw ("Verified KRISHNA takeover ran, but port 8766 is still occupied. listener_pid={0}" -f $remainingPid)
      }
    }else{
      $listenerRow=$null
  ...
  foreach($row in $chain){
    if(([string]$row.command_line) -match '(?i)KRISHNA_GUARDIAN\.ps1'){
      $root=$row
    }
  }
  if(!$root){
    foreach($row in $chain){
      if(([string]$row.command_line) -match '(?i)START_KRISHNA\.ps1'){
        $root=$row
      }
    }
  }
  if(!$root){
    foreach($row in $chain){
      if(([string]$row.executable_path).StartsWith((Join-Path $runtime ".venv"),[StringComparison]::OrdinalIgnoreCase)){
        $root=$row
      }
    }
  }
