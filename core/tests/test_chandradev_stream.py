...
from krishna_core.chandradev_stream import (
    ChandradevStreamRuntime,
...
class ChandradevStreamTests(unittest.TestCase):
    def runtime(self,root,**kwargs):
        return ChandradevStreamRuntime(
            Path(root)/"chandradev",
    ...
    def test_connection_guide_matches_original_osmo_limit(self):
        with tempfile.TemporaryDirectory() as td:
            v=self.runtime(td)
            guide=v.connection_guide("10.0.0.5")
    ...
    def test_status_is_honest_when_tools_are_not_installed(self):
        with tempfile.TemporaryDirectory() as td:
            v=self.runtime(td)
            status=v.status()
            self.assertEqual(status["agent"],"CHANDRADEV")
    ...
    def test_can_start_hawkeye_session(self):
        with tempfile.TemporaryDirectory() as td:
            v=self.runtime(td,hawkeye=FakeHawkeye())
            row=v.start_hawkeye_session(project="KRISHNA",scene_hint="workshop")
