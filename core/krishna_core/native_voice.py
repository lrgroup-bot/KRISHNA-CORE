...
class SanskritChantTTS:
    """Isolated local Sanskrit śloka recitation boundary.

    The configured worker is intentionally separate from Indic-TTS so KRISHNA
    never pretends Hindi synthesis is Sanskrit. Configure
    KRISHNA_SANSKRIT_TTS_CMD with {text}, {output} and optionally {meter}.
    """
    def __init__(self,command=None):
    ...
    def status(self):
        return {
            ...
            "note":"Sanskrit recitation has a dedicated local worker; Hindi/Odia TTS is not used as a Sanskrit substitute.",
        }
