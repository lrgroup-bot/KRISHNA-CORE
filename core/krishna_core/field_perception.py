...
class PerceptionCapability:
    ...
    def as_dict(self):
        ...
        d["limits"]=list(self.limits)
        return d


class FieldPerceptionPolicy:
    """Safety and capability policy for KRISHNA live-camera perception.

    This layer describes what the local vision stack may extract from live frames
    and what must remain redacted/unknown. It is intentionally provider-neutral.
    """
    ...
    CAPABILITIES=(
        ...
        PerceptionCapability(
            ...
            (
                "sensitive authentication secrets are always redacted",
            ),
        ),
        PerceptionCapability(
            ...
            (
                ...
                "OBD/CAN or measurement evidence is required for hidden faults",
            ),
        ),
        PerceptionCapability(
