...
class Handler(BaseHTTPRequestHandler):
    ...
    def _get(self):
        ...
        if path == "/api/kabach/privacy/history":
            ...
            return self._json(200,{"history":orch.kabach.privacy.history(limit),"count":len(orch.kabach.privacy.history(limit))})
    ...
    def _post(self):
        ...
        if post_path == "/api/kabach/privacy/audit":
            ...
            payload={
                ...
                "policy":str(data.get("policy") or "STANDARD"),
                "mission_id":data.get("mission_id"),
            }
            try:
                receipt=orch.dispatch_action(
                    "kabach.privacy.audit",payload,project=project,source="pc",actor="privacy-http",
        ...
        if post_path == "/api/kabach/privacy/clean-url":
            ...
            try:
                receipt=orch.dispatch_action(
                    "kabach.privacy.clean_url",{"url":str(data.get("url") or "")},
        ...
        if post_path == "/api/kabach/privacy/baseline":
            ...
            try:
                receipt=orch.dispatch_action(
                    ...
                    {"name":str(data.get("name") or ""),"report":data.get("report") or {}},
                    project=project,source="pc",actor="privacy-http",permissions=("privacy.write",),
                )
                return self._json(200,receipt["result"])
            except (ValueError,PermissionError) as exc:return self._json(400,{"error":str(exc)})
        ...
        if post_path == "/api/kabach/privacy/compare":
            ...
            try:
                receipt=orch.dispatch_action(
                    ...
                    {"name":str(data.get("name") or ""),"report":data.get("report") or {},
                     "mission_id":data.get("mission_id"),"configuration_change":data.get("configuration_change")},
                    project=project,source="pc",actor="privacy-http",permissions=("privacy.read",),
                )
                return self._json(200,receipt["result"])
            except KeyError as exc:return self._json(404,{"error":str(exc)})
        ...
        if post_path == "/api/kabach/privacy/release-gate":
            ...
            try:
                receipt=orch.dispatch_action(
                    ...
                    {"report":data.get("report") or {},"policy":str(data.get("policy") or "STANDARD")},
                    project=project,source="pc",actor="privacy-http",
