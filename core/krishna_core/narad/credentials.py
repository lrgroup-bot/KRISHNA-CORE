...
class NaradCredentialVault:
    ...
    def _validate_common(name, provider, header, scheme):
        ...
        header = str(header or "Authorization").strip()
    ...
    def register(self, name, provider, env_var, header="Authorization", scheme="Bearer"):
        ...
        if not env_var.replace("_", "").isalnum():
            raise ValueError("invalid env_var")
    ...
    def _available(self, ref: CredentialRef):
        if ref.source == "env":
            return bool(ref.env_var and os.getenv(ref.env_var))
        if ref.source == "vault":
            try:
    ...
    def resolve(self, credential_id):
        ...
        if ref.source == "env":
            ...
            if not secret:
                raise RuntimeError(f"credential environment variable is unavailable: {ref.env_var}")
            return secret
        if ref.source == "vault":
