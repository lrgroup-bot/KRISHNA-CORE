...
class Orchestrator:
    def __init__(self, db_path=None):
        ...
        self.vishvakarma_learning = self.agi.vishvakarma_learning
        self.sudarshan_design = self.agi.design
        self.sudarshan_ui = self.agi.ui_pipeline
    ...
    def _register_shared_actions(self):
        ...
        def design_plan_action(payload,context):
            return self.sudarshan_design.plan(DesignJob(
        ...
        def design_drift_action(payload,context):
            return self.sudarshan_design.drift.compare(payload.get("expected") or {},payload.get("actual") or {})

        def design_acceptance_action(payload,context):
            return self.sudarshan_design.acceptance.evaluate(payload.get("checks") or {})
        ...
        def design_ui_next_action(payload,context):
            ...
            return self.sudarshan_ui.next_action(payload.get("checks") or {},evidence)

        def vishvakarma_retrieve_action(payload,context):
            return {
                "findings":self.vishvakarma.retrieve(
                    str(payload.get("topic") or "design"),
                    limit=int(payload.get("limit") or 20),
                    verified_only=bool(payload.get("verified_only",True)),
                ),
                "status":self.vishvakarma.status(),
            }
        ...
        def vishvakarma_learn_action(payload,context):
            ...
            return self.vishvakarma_learning.ingest(lesson)
        ...
        def vishvakarma_verify_action(payload,context):
            ...
            return self.vishvakarma_learning.verify(lesson)
        ...
        self.action_bus.register(
            "design.plan",design_plan_action,description="Build a Sudarshan design plan with verified Vishvakarma context",
        ...
        self.action_bus.register(
            "vishvakarma.retrieve",vishvakarma_retrieve_action,description="Retrieve provenance-backed Rishi Vishvakarma design knowledge",
            permissions=("design.read","memory.read"),sources=("pc","system","agent","job","mcp","a2a"),
        )
        self.action_bus.register(
            "vishvakarma.learn",vishvakarma_learn_action,description="Store a candidate Vishvakarma design lesson for BRAHMA review",
        ...
        self.action_bus.register(
            "model.scout.evaluate",model_scout_evaluate_action,description="Evaluate a benchmarked local model candidate without downloading or routing it",
