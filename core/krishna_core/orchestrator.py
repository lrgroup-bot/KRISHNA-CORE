...
class Orchestrator:
    ...
    def _register_shared_actions(self):
        ...
        def narada_legal_corpus_plan_action(payload,context):
            return self.narada_legal.deep_corpus_plan(str(payload.get("jurisdiction") or "India"))
        ...
        self.action_bus.register(
            ...
            description="Read the bounded resumable Indian legal-corpus acquisition and completeness plan",
            permissions=("runtime.read",),sources=("pc","system","agent","job","mcp","a2a"),
        )
        self.action_bus.register(
...
"""
    ...
    def handle(self, message, project="general", source="pc", chat_id=None, vision_evidence=None):
        ...
        if self.narada_legal.looks_legal(message):
            legal_context=self.narada_legal.analysis_plan(message,"India")
            research_rows=[]
            try:
                queries=self.narada_legal.official_research_queries(message,"India")
                roles=["legal","illegal"]
                ...
                for role in list(dict.fromkeys(roles))[:3]:
                    report=self.garuda.scout(project,queries[role],5)
...
- Prefer current official Indian sources and authentic judgment text over summaries or repositories.
