"""Sudarshan-owned six-layer Project Brain contract."""
from pathlib import Path
import json,time
LAYERS=("PROJECT.md","ARCHITECTURE.md","RULES.md","PHASES.md","DESIGN.md","MEMORY.md")
SECTIONS={
...
class ProjectBrain:
 ...
 def record(self,project,section,message):
  ...
  with (p/"MEMORY.md").open("a",encoding="utf-8") as h:h.write(f"\n### {section} - {time.strftime('%Y-%m-%d %H:%M:%S')}\n{message.strip()}\n")
  self._state(p,project)
 def _state(self,p,project):
  (p/"PROJECT_STATE.json").write_text(json.dumps({"schema":"krishna.project-brain.v1","project":project,"governance_owner":"Sudarshan","layers":list(LAYERS),"updated_at":time.time()},indent=2),encoding="utf-8")
