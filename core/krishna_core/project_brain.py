...
Verified DB memory remains canonical for learned project context. Structured
project-governance layers live under KRISHNA runtime state, never inside a project
source tree unless an explicit future migration says otherwise.
"""
...
class ProjectBrain:
    ...
    def _project_dir(self,project):
        ...
        target=(root/self._slug(project)/".krishna").resolve()
        if target!=root and root not in target.parents:
            raise PermissionError("Project Brain governance path escaped runtime root")
        return target

    def _state(self,path,project):
        state={
            "schema":self.VERSION,
            "project":str(project),
        ...
        target=path/"PROJECT_STATE.json"
    ...
    def provision(self,project):
        ...
        self._state(path,project)
        return self.status(project)

    def status(self,project):
        path=self._project_dir(project)
        missing=[name for name in LAYERS if not (path/name).is_file()]
        state=path/"PROJECT_STATE.json"
        return {
            "project":str(project),
            "root":str(path),
    ...
    def record(self,project,section,message):
        ...
        with (path/"MEMORY.md").open("a",encoding="utf-8") as handle:
            handle.write(
                "\n### "+section+" - "+time.strftime("%Y-%m-%d %H:%M:%S")+"\n"+
                message[:12000]+"\n"
            )
        self._state(path,project)
        return {"recorded":True,"project":str(project),"section":section,"root":str(path)}
    ...
    def learn_verified(self,project,goal,result):
        ...
        if self.root is not None:
            try:
                self.record(project,"Completed",f"{goal}\n\nVerified result: {result}")
            except Exception:
                self.memory.audit("project_brain","governance_record_failed",str(project))
