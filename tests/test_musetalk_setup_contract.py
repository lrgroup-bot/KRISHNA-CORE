...
class MuseTalkSetupContractTests(unittest.TestCase):
    def test_gtx1050ti_setup_is_e_drive_cuda118_and_weight_verified(self):
        ...
        self.assertIn('musetalk_cuda_probe.py',text)
        self.assertIn('[System.IO.File]::WriteAllText',text)
        self.assertIn('major, minor = torch.cuda.get_device_capability(0)',text)
        self.assertIn('"capability": f"{major}.{minor}"',text)
        self.assertNotIn('$envPython -c @"',text)
        self.assertIn('[string[]]$CommandArgs',text)
        self.assertIn('& $Exe @CommandArgs',text)
        self.assertNotIn('[string[]]$Args',text)
        self.assertNotIn('& $Exe @Args',text)
        self.assertIn('Assert-EPath',text)
        self.assertIn('Get-FileHash -Algorithm SHA256',text)
        self.assertIn('Invoke-Checked -Exe $uvExe -CommandArgs @("self","version") -Label "Verify portable uv"',text)
